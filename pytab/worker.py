...
def workMan(worker_count, ffmpeg_cmd):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=worker_count) as executor:
        futures = {executor.submit(run_ffmpeg, nr, ffmpeg_cmd): nr for nr in range(worker_count)}
    ...
    for pid in range(worker_count):
        worker_output =run_ffmpeg(pid, ffmpeg_cmd) #Saving RAW-Data first, to save time during process-spawning
        if worker_output[1]:
            raw_worker_data = [None] #Deleting all the Raw Data, since run with failed Worker is not counted
        ...
        for pid in range(worker_count):
            ...
            for line in process_output.split("\n"):
                ...
                if re.match(r"^bench: maxrss", line):
                    rssline = line.split()
                    workrss = float(rssline[1].split("=")[-1].replace("kB", ""))                               #maxrss
            ...
            for line in framelines:
                new_line = line.split()
                frames.append(int(new_line[0].split("=")[-1]))
                framerates += int(new_line[1].split("=")[-1])
                speeds.append(float(new_line[6].split("=")[-1].replace("x", "")))

            avgSpeed = sum(speeds)/len(framelines)
            maxFrame = max(frames)
            avgFPS = framerates/len(framelines)
